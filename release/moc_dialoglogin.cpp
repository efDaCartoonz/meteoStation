/****************************************************************************
** Meta object code from reading C++ file 'dialoglogin.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.7.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../dialoglogin.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'dialoglogin.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.7.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_DialogLogin_t {
    QByteArrayData data[11];
    char stringdata0[136];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_DialogLogin_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_DialogLogin_t qt_meta_stringdata_DialogLogin = {
    {
QT_MOC_LITERAL(0, 0, 11), // "DialogLogin"
QT_MOC_LITERAL(1, 12, 11), // "loginAccept"
QT_MOC_LITERAL(2, 24, 0), // ""
QT_MOC_LITERAL(3, 25, 16), // "AutorizationData"
QT_MOC_LITERAL(4, 42, 7), // "logined"
QT_MOC_LITERAL(5, 50, 11), // "loginDenied"
QT_MOC_LITERAL(6, 62, 10), // "closeEvent"
QT_MOC_LITERAL(7, 73, 12), // "QCloseEvent*"
QT_MOC_LITERAL(8, 86, 5), // "event"
QT_MOC_LITERAL(9, 92, 19), // "on_btnLogin_clicked"
QT_MOC_LITERAL(10, 112, 23) // "on_btnAutoLogin_clicked"

    },
    "DialogLogin\0loginAccept\0\0AutorizationData\0"
    "logined\0loginDenied\0closeEvent\0"
    "QCloseEvent*\0event\0on_btnLogin_clicked\0"
    "on_btnAutoLogin_clicked"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_DialogLogin[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   39,    2, 0x06 /* Public */,
       5,    0,   42,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       6,    1,   43,    2, 0x08 /* Private */,
       9,    0,   46,    2, 0x08 /* Private */,
      10,    0,   47,    2, 0x08 /* Private */,

 // signals: parameters
    QMetaType::Void, 0x80000000 | 3,    4,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void, 0x80000000 | 7,    8,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void DialogLogin::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        DialogLogin *_t = static_cast<DialogLogin *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->loginAccept((*reinterpret_cast< AutorizationData(*)>(_a[1]))); break;
        case 1: _t->loginDenied(); break;
        case 2: _t->closeEvent((*reinterpret_cast< QCloseEvent*(*)>(_a[1]))); break;
        case 3: _t->on_btnLogin_clicked(); break;
        case 4: _t->on_btnAutoLogin_clicked(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (DialogLogin::*_t)(AutorizationData );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&DialogLogin::loginAccept)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (DialogLogin::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&DialogLogin::loginDenied)) {
                *result = 1;
                return;
            }
        }
    }
}

const QMetaObject DialogLogin::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_DialogLogin.data,
      qt_meta_data_DialogLogin,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *DialogLogin::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *DialogLogin::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_DialogLogin.stringdata0))
        return static_cast<void*>(const_cast< DialogLogin*>(this));
    return QDialog::qt_metacast(_clname);
}

int DialogLogin::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 5)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void DialogLogin::loginAccept(AutorizationData _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void DialogLogin::loginDenied()
{
    QMetaObject::activate(this, &staticMetaObject, 1, Q_NULLPTR);
}
QT_END_MOC_NAMESPACE
